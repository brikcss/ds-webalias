...
ConfigGen.prototype.createBanner = function createBanner () {
  return `/*! ${pkg.name} | @author ${pkg.author || 'Brikcss (https://github.com/brikcss)'} | @reference (${pkg.homepage || 'https://github.com/brikcss'}) */\n`
}
...
ConfigGen.prototype.createOutput = function createOutput (config, globals = {}) {
  ...
  config.output = config.output.map(o => {
    ...
    if (!o.file) {
      ...
      if (config.isParent) {
        o.file = o.file = pkg[config.target ? (self.options.pkgMap[`${o.format}:${config.type}`] || self.options.pkgMap[o.format] || 'main') : 'main']
      } else {
        o.file = `${self.options.outputDir ? `${self.options.outputDir}/` : ''}${o.format}/${config.id}.js`
      }
...
ConfigGen.prototype.create = function create (configs, globals = {}) {
  ...
  configs = configs.reduce((result, config) => {
    if (config.type && self.sets[config.type]) {
      return result.concat(self.sets[config.type].map(c => {
